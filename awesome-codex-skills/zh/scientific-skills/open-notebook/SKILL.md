---
name: open-notebook
description: Google NotebookLM 的自托管开源替代方案，用于 AI 驱动的研究和文档分析。适用于将研究资料组织到笔记本中、摄取多样化内容源（PDFs、视频、音频、网页、Office 文档）、生成 AI 驱动的笔记和摘要、基于研究创建多说话人播客、使用上下文感知 AI 与文档聊天、通过全文和向量搜索跨资料检索，或运行自定义内容转换。通过自托管实现完整数据隐私，支持 16+ 个 AI 提供商，包括 OpenAI、Anthropic、Google、Ollama、Groq 和 Mistral。
license: MIT
metadata:
...
Open Notebook 是 Google NotebookLM 的开源自托管替代方案，使研究人员能够组织资料、生成 AI 驱动的洞察、创建播客，并与自己的文档进行上下文感知对话，同时保持完整数据隐私。

与 Google 的 Notebook LM 不同，后者在 Enterprise 版本之外没有公开可用的 API；Open Notebook 提供完整的 REST API，支持 16+ 个 AI 提供商，并完全运行在你自己的基础设施上。

**相较 NotebookLM 的主要优势：**
- 用于编程访问和自动化的完整 REST API
- 可选择 16+ 个 AI 提供商（不锁定到 Google 模型）
- 可自定义 1-4 位说话人的多说话人播客生成（相比 2 位说话人限制）
- 通过自托管实现完整数据主权
...
- 已安装 Docker Desktop
- 至少一个 AI 提供商的 API key（或使用本地 Ollama 进行免费的本地推理）
...
2. 为首选提供商（OpenAI、Anthropic 等）添加凭据
3. 测试连接并发现可用模型
4. 注册模型以供整个平台使用
...
### 播客生成
从研究资料生成专业的多说话人播客，支持 1-4 位可自定义说话人。
...
Open Notebook 通过 Esperanto library 支持 16+ 个 AI 提供商：
...
- **前端：** Next.js 与 React
- **部署：** Docker Compose 与持久化卷

## 重要注意事项

- Open Notebook 需要 Docker 才能部署
- 必须配置至少一个 AI 提供商，AI 功能才能工作
