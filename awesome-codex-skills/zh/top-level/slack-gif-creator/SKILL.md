...
description: 用于创建针对 Slack 优化的 animated GIFs 的工具包，包含 size constraints validators 和可组合 animation primitives。当用户请求为 Slack 创建 animated GIFs 或 emoji animations，例如 “make me a GIF for Slack of X doing Y” 时使用。
license: 完整条款见 LICENSE.txt
---

# Slack GIF Creator - 灵活工具包

用于创建针对 Slack 优化的 animated GIFs 的工具包。提供 Slack 约束的 validators、可组合 animation primitives，以及可选 helper utilities。**根据实现创意愿景的需要灵活应用这些工具。**

## Slack 的要求

Slack 会根据用途对 GIFs 提出具体要求：
...
**Emoji GIFs 具有挑战性** - 64KB 限制很严格。有帮助的策略：
- 总帧数限制在 10-15 帧
- 最多使用 32-48 种颜色
- 保持设计简单
- 避免 gradients
...
1. **Validators** - 检查 GIF 是否满足 Slack 的要求
2. **Animation Primitives** - 可组合的 motion building blocks（shake、bounce、move、kaleidoscope）
3. **Helper Utilities** - 面向常见需求的可选函数（text、colors、effects）

**这些工具的应用方式拥有完全的创作自由。**

## Core Validators

为确保 GIF 满足 Slack 的约束，请使用这些 validators：
...
这些是可组合的 motion building blocks。可以将它们以任意组合应用到任何对象：
...
**若要自由组合 primitives，请遵循这些 patterns：**
```python
...
这些是面向常见需求的可选 helpers。**根据需要使用、修改，或用 custom implementations 替换。**
...
对于 emoji 这类小 GIFs，文本可读性很有挑战。常见解决方式是添加 outlines：
...
要实现 custom text rendering，请使用 PIL 的 `ImageDraw.text()`，它对较大的 GIFs 效果很好。

### Color Management

专业外观的 GIFs 通常使用协调的 color palettes：
...
要直接处理 colors，请使用 RGB tuples，按使用场景选择即可。

### Visual Effects

用于 impact moments 的可选 effects：
...
平滑 motion 使用 easing，而不是 linear interpolation：
...
可用 easings：`linear`、`ease_in`、`ease_out`、`ease_in_out`、`bounce_out`、`elastic_out`、`back_out`（overshoot），以及 `core/easing.py` 中的更多内容。

### Frame Composition

如果需要，可以使用基础 drawing utilities：
...
**Message GIFs（>2MB）：**
1. 减少帧数（降低 FPS 或缩短时长）
2. 减少颜色（128 -> 64 colors）
3. 减小尺寸（480x480 -> 320x320）
4. 启用 duplicate frame removal

**Emoji GIFs（>64KB）- 要激进优化：**
1. 总帧数限制在 10-12 帧
2. 最多使用 32-40 种颜色
3. 避免 gradients（solid colors 压缩效果更好）
4. 简化设计（更少元素）
5. 在 save method 中使用 `optimize_for_emoji=True`
...
此工具包提供 building blocks，而不是僵硬 recipes。处理 GIF 请求时：

1. **理解创意愿景** - 应该发生什么？是什么 mood？
2. **设计动画** - 将其拆分成 phases（anticipation、action、reaction）
3. **按需应用 primitives** - Shake、bounce、move、effects，自由混合
4. **验证约束** - 检查文件大小，尤其是 emoji GIFs
5. **必要时迭代** - 如果超过大小限制，减少 frames/colors

**目标是在 Slack 技术约束内保持创作自由。**

## Dependencies

使用此工具包时，仅在尚未安装这些 dependencies 时安装：
