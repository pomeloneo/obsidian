...
原文链接：https://time.geekbang.org/column/article/936334

---

[![](https://static001.geekbang.org/resource/image/d3/57/d309af2894e6bddd4a841851c4beee57.jpg)](https://static001.geekbang.org/resource/image/d3/57/d309af2894e6bddd4a841851c4beee57.jpg)
...
[![](https://static001.geekbang.org/resource/image/b1/9a/b178fb4125eb0dc5e0e33ef1c7yye89a.png?wh=1646x706)](https://static001.geekbang.org/resource/image/b1/9a/b178fb4125eb0dc5e0e33ef1c7yye89a.png?wh=1646x706)
...
1. *“合宪性”审查:** 逐条对照`constitution.md`的原则，检查并确认本技术方案符合所有条款（特别是包内聚、错误处理、TDD）。

1. *项目结构细化:** 明确`cmd/`, `internal/github/`, `internal/converter/`等包的具体职责和依赖关系。

1. *核心数据结构:** 定义在模块间流转的核心Go `struct`，例如`IssueData`，必须包含Spec中要求的所有字段（Title, Author, Reactions等）。

1. *接口设计:** 定义`internal`包对外暴露的关键Interface。
...
1. *TDD强制：** 根据`constitution.md`的“测试先行铁律”，**必须**先生成测试任务，后生成实现任务。

1. *并行标记：** 对于没有依赖关系的任务，请标记 `[P]`。

1. *阶段划分：** 即便`plan.md`中包含了粗略的阶段划分，也要以下面的为准。
...
惊人的细粒度：AI 将整个项目拆解成了 180+ 个原子任务！每一个结构体的定义、每一个方法的实现，都被拆分成了独立的任务。

严格的 TDD 执行：请注意看任务编号。奇数编号的任务（如 T37, T65）几乎全是创建测试，偶数编号（如 T43, T69）才是实现代码。这完美贯彻了我们“宪法”中的“测试先行”铁律。

并行性识别：大量的 [P] 标记出现在了基础结构定义和无依赖的工具函数中，这意味着如果我们有多个 AI Agent，它们可以同时开工。

阶段清晰：从 Foundation 到 URL Parser，再到 GitHub Fetcher 和 Converter，最后是 CLI 集成，层次分明，逻辑严密。

在你批准后，我们就完成了从“蓝图”到“施工清单”的全部转化。现在，AI 已经拥有了一份精确到“先写测试 T036，再写代码 T043”级别的详细指令。

【再次强调：审查你的“任务指令集”】

同样地，这份 tasks.md 也需要你的严格审查。虽然 180+ 个任务看起来很多，但你只需要检查关键路径和依赖关系。
...
我们今天生成的 tasks.md 中，包含了 [P] 这个并行标记。请你思考一下，在真实的团队协作或 CI/CD 环境中，这个 [P] 标记，除了指导单个 AI Agent 可以并行思考之外，还有哪些更宏观的、更具颠覆性的工程价值？它对于我们组织开发流程、分配人力，乃至未来的“多 Agent 协同”范式，可能带来哪些启发？
