...
原文链接：https://time.geekbang.org/column/article/934402

---

[![](https://static001.geekbang.org/resource/image/b9/62/b9deb0f1edfce0988bdabb27c2e61a62.jpg)](https://static001.geekbang.org/resource/image/b9/62/b9deb0f1edfce0988bdabb27c2e61a62.jpg)
...
[![](https://static001.geekbang.org/resource/image/a1/6d/a137bfb9121656c135fba2cf679c426d.jpg?wh=1902x1538)](https://static001.geekbang.org/resource/image/a1/6d/a137bfb9121656c135fba2cf679c426d.jpg?wh=1902x1538)
...
Subagent 这个听起来很强大的概念，在 Claude Code 中的实现却异常优雅和简洁。一个 Subagent 的全部定义，都浓缩在一个带有 YAML Frontmatter 的 Markdown 文件中。
...
tools（可选）：这是一个权限定义。你可以在这里为这个 Subagent 单独配置它能使用的工具集。这遵循了我们在第 9 讲中学到的“最小权限原则”，是实现安全性的关键。
...
虽然你可以手动创建和编辑这些.md文件，但 Claude Code 提供了一个更友好、更不容易出错的管理方式——/agents 指令。
...
[![](https://static001.geekbang.org/resource/image/25/72/25b585bcbe9e7cd7fyyd6d7729598072.png?wh=2876x892)](https://static001.geekbang.org/resource/image/25/72/25b585bcbe9e7cd7fyyd6d7729598072.png?wh=2876x892)

编辑（Edit）：可视化地修改一个现有 Subagent 的 description、tools 等属性。尤其是在配置 tools 时，它会列出所有可用的工具让你勾选，非常方便。

删除（Delete）：安全地删除一个不再需要的 Subagent。

[![](https://static001.geekbang.org/resource/image/ce/5c/ceca5d4f1621dc9b5ef74be735bd8b5c.png?wh=2874x494)](https://static001.geekbang.org/resource/image/ce/5c/ceca5d4f1621dc9b5ef74be735bd8b5c.png?wh=2874x494)

对于初学者来说，强烈建议通过 /agents 命令来创建你的第一个 Subagent。它能确保文件格式的正确性，并让你对所有可配置项有一个直观的了解。

此外，在上一讲，我们已经从调用模式的维度，系统性地对比了 Slash Commands（用户调用）、Agent Skills（AI 自主发现）和 Subagents（AI 主动委托）。现在，当我们聚焦于 Subagent 时，必须抓住它与前两者最根本、最核心的区别，那就是——独立的上下文窗口（Isolated Context）。Subagent 的核心价值在于上下文隔离和深度专注，这是其他两者无法替代的。
...
[![](https://static001.geekbang.org/resource/image/50/4d/50f45c03509ce5c7f2933e3dca2d974d.png?wh=2874x930)](https://static001.geekbang.org/resource/image/50/4d/50f45c03509ce5c7f2933e3dca2d974d.png?wh=2874x930)

### 第二步：创建新 Agent

在菜单中，选择 Create New Agent，系统会询问你希望创建 Project-level（项目级）还是 User-level（用户级）的 Subagent：

[![](https://static001.geekbang.org/resource/image/0b/a9/0b600d20bb14c6567cb420a7d38c30a9.png?wh=2880x474)](https://static001.geekbang.org/resource/image/0b/a9/0b600d20bb14c6567cb420a7d38c30a9.png?wh=2880x474)
...
[![](https://static001.geekbang.org/resource/image/77/09/7758f47e44a62yycf33c441a174e5d09.png?wh=2872x486)](https://static001.geekbang.org/resource/image/77/09/7758f47e44a62yycf33c441a174e5d09.png?wh=2872x486)

我们选择“手工配置”，然后进入下一个页面，配置该 Subagent 的 ID：

[![](https://static001.geekbang.org/resource/image/6a/ce/6a50865b86d1a0009e9d116dd45110ce.png?wh=2878x586)](https://static001.geekbang.org/resource/image/6a/ce/6a50865b86d1a0009e9d116dd45110ce.png?wh=2878x586)

这是 Subagent 的唯一 ID，使用小写字母和连字符。我们输入：go-code-security-reviewer。

下一步配置该 Subagent 的 System Prompt（系统提示）：

[![](https://static001.geekbang.org/resource/image/79/46/79a9cb6f9ac00ee3a7140d5bd9e1cd46.png?wh=2870x1612)](https://static001.geekbang.org/resource/image/79/46/79a9cb6f9ac00ee3a7140d5bd9e1cd46.png?wh=2870x1612)
...
接下来，进入 description 信息配置页，这是最至关重要的一步！description 是主 Agent 用来自主发现和决定是否委托这个 Subagent 的核心依据。它必须清晰、准确，并包含丰富的“触发关键词”：

[![](https://static001.geekbang.org/resource/image/35/46/351ceacdd1a3ca087decba009383c046.png?wh=2868x588)](https://static001.geekbang.org/resource/image/35/46/351ceacdd1a3ca087decba009383c046.png?wh=2868x588)
...
[![](https://static001.geekbang.org/resource/image/a7/cd/a71f5c49b100a2a67dd4363257a84ccd.png?wh=2840x1400)](https://static001.geekbang.org/resource/image/a7/cd/a71f5c49b100a2a67dd4363257a84ccd.png?wh=2840x1400)
...
[![](https://static001.geekbang.org/resource/image/01/b4/01e02f01a2c8b8c9a87ede7faa5bbfb4.png?wh=2870x540)](https://static001.geekbang.org/resource/image/01/b4/01e02f01a2c8b8c9a87ede7faa5bbfb4.png?wh=2870x540)

[![](https://static001.geekbang.org/resource/image/88/c1/88354aea1cabfd280eb4bdbb963ed2c1.png?wh=2864x808)](https://static001.geekbang.org/resource/image/88/c1/88354aea1cabfd280eb4bdbb963ed2c1.png?wh=2864x808)

[![](https://static001.geekbang.org/resource/image/07/c8/078yy0d41be0c08349dd96bc4860eec8.png?wh=2868x1178)](https://static001.geekbang.org/resource/image/07/c8/078yy0d41be0c08349dd96bc4860eec8.png?wh=2868x1178)
...
你会看到，主 Agent 在思考后，go-code-security-reviewer Subagent 会被激活，在它自己的、干净的上下文中，开始执行我们在其 System Prompt 中定义的专业审查流程。最终，它会将一份结构化的报告返回给主 Agent，再由主 Agent 呈现给你：

[![](https://static001.geekbang.org/resource/image/93/47/933d210a831b7768da7a8d3df5ace647.png?wh=2188x1584)](https://static001.geekbang.org/resource/image/93/47/933d210a831b7768da7a8d3df5ace647.png?wh=2188x1584)
...
[![](https://static001.geekbang.org/resource/image/d8/bd/d899ca747ce3069604da50561e2291bd.png?wh=2616x1566)](https://static001.geekbang.org/resource/image/d8/bd/d899ca747ce3069604da50561e2291bd.png?wh=2616x1566)
...
[![](https://static001.geekbang.org/resource/image/0c/4c/0c33b2e0c0ff1da776cf09850ebf0c4c.png?wh=1854x1374)](https://static001.geekbang.org/resource/image/0c/4c/0c33b2e0c0ff1da776cf09850ebf0c4c.png?wh=1854x1374)

### 先让 AI 创造，再由你升华

像我们上面示例中从一张白纸开始编写一个完美的 Subagent System Prompt 是一件困难的事。最佳的起点，是利用 AI 来创造 AI。
...
Subagent 的核心价值在于其专业性，而非通用性。抵制住创造一个“什么都能干”的超级 Agent 的诱惑。
...
这是我们在第 9 讲就反复强调的安全基石。对于 Subagent 来说，这一点尤为重要，因为它可能在你看不到的后台被主 Agent 自动调用。
...
Project 级的 Subagent（即存放在 ./.claude/agents/ 中的）是你团队的宝贵数字资产。
...
首先，我们从一个复杂任务场景出发，分析了单一 Agent 模型在处理多维度问题时的瓶颈，并引入了多智能体系统作为解决方案。我们理解了其通过上下文压缩与关注点分离来提升性能和可靠性的核心优势。

接着，我们将 Subagent 定位为构建多智能体系统的核心原子能力，并深入剖析了它在 Claude Code 中的具象化实现：一个由 YAML Frontmatter（定义 name, description, tools, model）和 Markdown 正文（定义 System Prompt）构成的标准文件。

然后，通过一个完整的实战，我们亲手创建并配置了一个专业的“Go 代码安全审查员”Subagent，并掌握了 description 对于实现 AI 自主委托的关键作用。

最后，我们简单介绍了高效调用与编排 Subagent 的三种模式：隐式调用、显式调用和强大的链式编排，学会了如何从“提问者”转变为“工作流编排者”。并了解了 Subagent 设计的几个最佳实践。
...
每个 Subagent 的名称和核心职责。

你（作为主 Orchestrator）会如何编排它们的工作顺序？
