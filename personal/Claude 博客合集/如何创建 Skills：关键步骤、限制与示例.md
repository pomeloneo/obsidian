...
[Skills](https://www.claude.com/blog/skills) 是为特定任务或领域扩展 Claude 能力的自定义指令。

当你通过 [SKILL.md](http://SKILL.md) 文件创建 Skill 时，你是在教 Claude 如何更有效地处理特定场景。Skills 的力量在于编码机构知识、标准化输出，以及处理复杂的多步骤工作流。

## **5 步创建 Skill**

### **1. 理解核心需求**

明确你的 Skill 解决什么问题。强大的 Skills 针对有可衡量结果的具体需求。

### **2. 编写名称**

名称应直接且描述性。使用小写加连字符（如 pdf-editor、brand-guidelines）。

### **3. 编写描述字段**

描述决定你的 Skill 何时激活，是最关键的组件。从 Claude 的角度编写，关注触发条件、能力和用例。

### **4. 编写主要指令**

指令应结构化、易扫描、可操作。使用 markdown 标题、项目符号和代码块。

### **5. 上传你的 Skill**

- [Claude.ai](http://Claude.ai)：在设置中添加自定义 Skill

- [Claude Code](https://www.claude.com/product/claude-code)：在项目根目录创建 skills/ 目录

- [Claude 开发者平台](https://www.claude.com/platform/api)：通过 Skills API 上传

## **通用最佳实践**

- **从用例开始**：不要投机性地编写 Skills，在有真实重复任务时构建

- **定义成功标准**：告诉 Claude 好的输出是什么样的

- **使用 Skill-Creator Skill**：引导你创建结构良好的 Skills

## **Skill 的限制和考虑**

- **Skill 触发**：Claude 语义评估 Skill 描述与请求的相关性

- **适当的文件大小**：避免不必要的上下文膨胀，使用"菜单"方法

## **开始使用**

[Claude.ai](http://Claude.ai) 用户：在设置 → 功能中启用 Skills

API 开发者：探索[文档](https://docs.anthropic.com/)
...
pandoc --track-changes=all path-to-file.docx -o [output.md](http://output.md)
...
pandoc --track-changes=all path-to-file.docx -o [current.md](http://current.md)
...
python ooxml/scripts/[pack.py](http://pack.py) unpacked reviewed-document.docx
...
pandoc --track-changes=all reviewed-document.docx -o [verification.md](http://verification.md)

```jsx
-Verify ALL changes were applied correctly:
```

grep "original phrase" [verification.md](http://verification.md) # Should NOT find it

grep "replacement phrase" [verification.md](http://verification.md) # Should find it
