...
多智能体系统是一种架构，其中多个 LLM 实例以独立的对话上下文运行，通过代码进行协调。存在多种协调模式（智能体群、基于能力的系统和消息总线架构），但本文重点关注编排者-子智能体模式：一种层次化模型，由主智能体为特定子任务生成和管理专门的子智能体。

目前，多智能体系统经常被应用在单个智能体表现更好的情况下，尽管随着模型改进，这一计算也在不断演变。在 Anthropic，我们看到团队花费数月构建复杂的多智能体架构，最终发现改进单个智能体的提示词就能达到同等效果。

在构建多智能体系统并与将其部署到生产环境的团队合作后，我们确定了三种多个智能体始终优于单个智能体的情况：当上下文污染降低性能时、当任务可以并行运行时，以及当专门化提高工具选择或任务聚焦时。在这些情况之外，协调成本通常超过收益。

## 从单个智能体开始的理由

设计良好的单个智能体配合适当的工具，能完成的工作远超许多开发者的预期。多智能体系统引入了开销。在我们的测试中，多智能体实现通常比等效任务的单智能体方法使用 3-10 倍的 Token。

## 多智能体系统的决策框架

### 上下文保护

当智能体的上下文积累了与后续子任务无关的信息时，就会发生上下文污染。子智能体提供隔离，每个都在自己专注于特定任务的干净上下文中运行。

### 并行化

并行运行多个智能体让你能探索比单个智能体更大的搜索空间。并行化的主要好处是彻底性而非速度。

### 专门化

不同任务有时受益于不同的工具集、系统提示词或专业领域。专门的智能体配合聚焦的工具集可以提高可靠性。包括工具集专门化、系统提示词专门化和领域专业知识专门化。

## 以上下文为中心的分解

采用多智能体架构时，最重要的设计决策是如何在智能体之间分配工作。关键洞察是采用**以上下文为中心的视角**而非以问题为中心的视角。

## 验证子智能体模式

一个在各领域始终表现良好的多智能体模式是**验证子智能体**——一个专门负责测试或验证主智能体工作的智能体。验证子智能体成功是因为它们避开了"电话游戏"问题。

### 早期胜利问题

验证子智能体最显著的失败模式是在未经彻底测试的情况下将输出标记为通过。缓解策略包括具体标准、全面检查、负面测试和明确指令。

## 展望

多智能体系统强大但并非普遍适用。在添加多个协调智能体的复杂性之前，确认：真正的约束存在、分解遵循上下文而非问题类型、存在清晰的验证点。

我们的建议？从最简单可行的方法开始，只在证据支持时才增加复杂性。
...
1. **Domain confusion.** When tools span multiple unrelated domains (database operations, API calls, file system operations), the agent confuses which domain applies to a given task.

1. **Degraded performance.** Adding new tools degrades performance on existing tasks, suggesting the agent has reached its capacity for tool management.
...
1. **Decomposition follows context, not problem type.** Group work by what context it requires, not by what kind of work it is.

1. **Clear verification points exist** where subagents can validate work without requiring full context.

Our advice? Start with the simplest approach that works, and add complexity only when evidence supports it.

_This is the first in a series of posts on multi-agent systems. For more on single-agent patterns, see [Building effective agents](https://www.anthropic.com/engineering/building-effective-agents). For context management strategies, see [Effective context engineering for AI agents](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents). For a deep dive into how we built our multi-agent research system, see [How we built our multi-agent research system](https://www.anthropic.com/engineering/multi-agent-research-system)._
